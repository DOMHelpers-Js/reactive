...
effect(() => {
  Elements.update({
    ...
    warning: { hidden: counter.count <= 10 }
  });
});
...
effect(() => {
  Elements.update({
    loginBtn:   { hidden: auth.isLoggedIn },
    logoutBtn:  { hidden: !auth.isLoggedIn },
    welcomeMsg: { textContent: auth.isLoggedIn ? `Welcome, ${auth.userName}!` : '' }
  });
});
...
effect(() => {
  ...
  Elements.update({
    emailError: {
      textContent: isEmailValid ? '' : 'Invalid email',
      hidden:      !form.email || isEmailValid
    },
    passwordError: {
      textContent: isPasswordValid ? '' : 'Min 8 characters',
      hidden:      !form.password || isPasswordValid
    },
...
effect(() => {
  ...
  Elements.completionMessage.update({ hidden: percentage < 100 });
});
...
effect(() => {
  ...
  Elements.cartBadge.update({ textContent: count, hidden: count === 0 });
